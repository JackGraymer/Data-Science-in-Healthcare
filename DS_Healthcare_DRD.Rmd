...
# Filters the data to only include drug-induced deaths (removes alcohol-induced deaths)
...
## time-series forecasting (e.g., ARIMA) to predict future death trends.
...
# Extract the time series data for Oregon
Data_oregon <- Data_monthly %>%
  filter(`Occurrence State` == "Oregon") %>%
  select(date, Deaths_per_100000)

# Create a time series object
oregon_ts <- ts(Data_oregon$Deaths_per_100000, start = c(2018, 1), frequency = 12)

# Fit an ARIMA model to the time series data
arima_model <- arima(oregon_ts, order = c(1, 1, 1))

# Generate forecasts for the next 12 months
forecast_values <- forecast(arima_model, h = 12)

# Plot the observed data and forecasted values
plot(forecast_values, main = "ARIMA Forecast for Drug-Related Deaths in Oregon",
     xlab = "Year", ylab = "Deaths per 100,000 inhabitants")
```

```{r Cross-Validation}	
# Cross-Validation: Validate the ARIMA model with part of the data
# Split the data into training (2018-2022) and validation (2023-2024) sets
training_data <- window(oregon_ts, start = c(2018, 1), end = c(2022, 12))
validation_data <- window(oregon_ts, start = c(2023, 1), end = c(2024, 12))

# Fit the ARIMA model to the training data
arima_model_cv <- arima(training_data, order = c(1, 1, 1))

# Generate forecasts for the validation period
forecast_values_cv <- forecast(arima_model_cv, h = 24)

# Plot the observed data, forecasted values, and validation data
plot(forecast_values_cv, main = "ARIMA Forecast Validation for Drug-Related Deaths in Oregon",
     xlab = "Year", ylab = "Deaths per 100,000 inhabitants")
lines(validation_data, col = "red")
legend("topleft", legend = c("Forecast", "Validation Data"), col = c("black", "red"), lty = 1)


```	

```{r Time-Series Forecasting TBATS}
# Extract the time series data for Oregon
Data_oregon <- Data_monthly %>%
  filter(`Occurrence State` == "Oregon") %>%
  select(date, Deaths_per_100000)
...
# Fit a TBATS model to the time series data
tbats_model <- tbats(oregon_ts)

# Generate forecasts for the next 12 months
forecast_values_tbats <- forecast(tbats_model, h = 12)

# Plot the observed data and forecasted values

plot(forecast_values_tbats, main = "TBATS Forecast for Drug-Related Deaths in Oregon",
     xlab = "Year", ylab = "Deaths per 100,000 inhabitants")
